...
function Home() {
  ...
  return (
    <div className='bg-[url("../../public/Images/6th.jpg")] h-screen w-screen py-4 relative overflow-hidden bg-cover'>
      ...
      <button onClick={handleHomeClick} className='text-5xl md:text-5xl font-poppins lg:text-6xl ml-[35px] text-white font-bold shadow-md rounded-xl w-[295px] px-4 animate-appear'><span className='font-outline-4'>Chat</span><span className='text-[#48cae4] animate-appear'>Box</span></button>
        <div className='md:mr-[65px] lg:mr-[100px] flex place-content-around mb-3  w-[350px] animate-appear'>
          <button onClick={handleLoginClick} className='mr-10 mb-4 text-white md:mr-6 text-2xl mt-4 md:text-xl lg:text-2xl hover:text-black hover:cursor-pointer rounded-full w-[80px] h-[45px] bg-[#48cae4] active:scale-[.98] active:duration-75  transition-all hover:scale-[1.05] ease-in-out cursor-pointer animate-appear'>Login</button>
          <button onClick={handleRegisterClick} className='ml-6 mb-4 text-white md:ml-2 text-2xl mt-4 md:text-xl lg:text-2xl hover:text-black hover:cursor-pointer rounded-full w-[110px] h-[45px] bg-[#48cae4] active:scale-[.98] active:duration-75  transition-all hover:scale-[1.05] ease-in-out cursor-pointer animate-appear'>Register</button>
        </div>
      </navbar>
      <div className='top-[185px] left-20 absolute font-extrabold text-3xl mb-2 text-balance w-full text-[#48cae4] animate-appear'>
        <Typewriter
        onInit={(typewriter) => {
          typewriter.typeString('Welcome to ChatBox -<br/> Where Every Conversation is Unique! animate-appear')
            .pauseFor(2500)
            .deleteAll()
            .start()
        }}
      ...
      /></div>
      <hero className='hidden md:block absolute top-[190px] left-20 bottom-[0] w-[40%] h-[70%] rounded-3xl animate-appear'>
        <p className='text-[#000000] font-semibold font-poppins tracking-wider leading-8 animate-appear'>
      ...
      <hero className='md:hidden absolute top-[170px] left-20 bottom-[0] w-[40%] h-[70%] rounded-3xl animate-appear'>
        <p className='text-[#010303] font-semibold font-poppins tracking-wider leading-8 animate-appear'>
